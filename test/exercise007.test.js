...
describe("sumDigits", () => {
  ...
  test("it returns the sum of all digits", () => {
    ...
    expect(sumDigits([1112312367564])).toBe(
      1 + 1 + 1 + 2 + 3 + 1 + 2 + 3 + 6 + 7 + 5 + 6 + 4
    );
  ...
  test("it should throw an error when the input is not a number", () => {
    expect(() => sumDigits("L")).toThrowError(
      "sumDigits(n): n must be a number"
    );
    expect(() => sumDigits("DAJSDLJKASDL")).toThrowError(
      "sumDigits(n): n must be a number"
    );
  });

  test("it should throw an error when the input is negative", () => {
    expect(() => sumDigits(-121231)).toThrowError(
      "sumDigits(n): n must be positive"
    );
    expect(() => sumDigits(-5)).toThrowError(
      "sumDigits(n): n must be positive"
    );
  });
});
...
describe("createRange", () => {
  ...
  test("works with mixes of integers and non-integers", () => {
    ...
    expect(createRange(1.5, 4, 0.5)).toStrictEqual([
  ...
  test("it should throw an error when either of the first two parammeters are missing", () => {
    ...
    expect(() => createRange("A", 1, 1)).toThrowError(
      "createRange(start,end,step=1): all parameters must be numbers"
    );
    expect(() => createRange(1, "A", "A")).toThrowError(
      "createRange(start,end,step=1): all parameters must be numbers"
    );
    expect(() => createRange("A", 1, "A")).toThrowError(
      "createRange(start,end,step=1): all parameters must be numbers"
    );
    expect(() => createRange("A", "A", 1)).toThrowError(
      "createRange(start,end,step=1): all parameters must be numbers"
    );
  });

  test("it should throw an error when the step is zero", () => {
    expect(() => createRange(1, 3, 0)).toThrowError(
      "createRange(start,end,step=1): step must be non-zero"
    );
  });
});
...
describe("hexToRGB", () => {
  ...
  test("it should throw an error when no parameter is passed", () => {
    expect(() => hexToRGB()).toThrowError(
      "hexToRGB(hexStr): hexStr is required"
    );
  });
});
...
describe("findWinner", () => {
  test("test of testBoards", () => {
    ...
    expect(findWinner(testboards[12].board) == testboards[12].winner).toBe(true);
    
});

  test("it should throw an error when no board is passed", () => {
    expect(() => findWinner()).toThrowError(
      "findWinner(board): board is required."
    );
  });
});
...
const userScreenTimeArray = [
    ...
    name: "Beth Smith",
    screenTime: [
        { date: "2019-05-01", usage: { twittergb: 34, instagram: 22, facebook: 40} },
        { date: "2019-05-02", usage: { twittergb: 56, instagram: 40, facebook: 31} },
        { date: "2019-05-03", usage: { twittergb: 12, instagram: 15, facebook: 19} },
        { date: "2019-05-04", usage: { twittergb: 10, instagram: 56, facebook: 61} },
        ]
    },
    {
    username: "sam_j_1989",
    ...
    name: "Billy Smith",
    screenTime: [
            { date: "2019-05-01", usage: { twittergb: 3, instagram: 2, facebook: 4} },
            { date: "2019-05-02", usage: { twittergb: 156, instagram: 140, facebook: 131} },
            { date: "2019-06-04", usage: { twittergb: 110, instagram: 156, facebook: 161} },
        ]
    },
    {
    ...
    screenTime: [
            ...
            { date: "2019-06-13", usage: { twittergb: 132, instagram: 105, facebook: 9} },
...
describe('getScreentimeAlertList', () => {
    ...
    test('test of test', () => {
        expect(getScreentimeAlertList(userScreenTimeArray, "2019-05-02")).toEqual(["beth_1234", "billy_1234"]);
    });
